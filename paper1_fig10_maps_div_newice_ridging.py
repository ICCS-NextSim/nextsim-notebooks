...
#Variables
vname='ridge_ratio' # 'divergence'  # sit_obs_remse_diff 
# sie, bsie,
...
serie_or_maps=[4]#[1:4] # 1=serie, 2=video, and 3=map, 4=smap, 0 for neither
my_dates=1
inc_obs=0
...
for serie_or_map in serie_or_maps:
  ...
  if serie_or_map==1: # series
    ...
    vnames=[ 'ridge_ratio','divergence','newice'] 
    varrays=['ridge_ratio','siv'       ,'newice'] 
  else:
...
#zzz
      ...
      if plot_smap==1:
        ...
        if vname=='sic':
          ...
          cmap = cmocean.cm.matter; vmin=0; vmax=1
          vnamee='ice growth (cm/day) '
        ...
        if ex==expt[-1]:
          ...
          #for ts in range(0,len(time_obsixn)):  
            ...
            if ts==len(time_obsixn)-1 and kmv==len(vnames)-1:  
              ...
              figname=path_fig+run+'/map_solo_div_newice_ridging'+'_'+format(time[t[0]].strftime('%Y_%m_%d_%H_%M'))+'.png'
              if save_fig==1:
                if os.path.exists(path_fig+run)==False:
                  os.mkdir(path_fig+run)
                print('Saving: '+figname)
