...
#Variables
vname='divergence'  # sit_obs_remse_diff 
# sie, bsie,
    ...
    for ex in expt:
      ...
      if plot_hist==1:
        ...
        if kmv==0 and ex==expt[0]:
          fig=plt.figure(figsize = (8,4)) # (8,9)) 
        # Plotting time series
        # Ice divergence
        if vname[0:10]=='divergence' or vname=='convergence': # if first expt load obs
          ...
          if ke==1:
            ...
            ax = fig.add_subplot(1,2, kmv+1) # landscape
            ll=[]
          ...
          if vname[0:10]=='divergence': # if first expt load obs
            if hist_norm==1: 
              ...
              color=colors[ke-1])
            ax.set_title('Divergence',fontsize=14,fontweight='bold')
            ax.set_xlabel('(1/day)',fontsize=14,fontweight='bold')
            plt.ylim([0, 4E6])
            ...
            #  ll=[]
            if hist_norm==1: 
              ax.semilogx(hcon[1][0:-1]*-1,hcon[0]/np.max(hcon[0][:]),
              color=colors[ke-1])
              #plt.ylim([0, 1])
            else:
              ax.semilogx(hcon[1][0:-1]*-1,hcon[0],
              color=colors[ke-1])
            #plt.ylim([0, 1E2])
            plt.ylim([0, 5E6])
        ...
        if vname=='newice' or vname=='ridge_ratio': # if first expt load obs
          ...
          if vname=='ridge_ratio': # if first expt load obs
            ...
            ax.semilogx(hdiv[1][0:-1],hdiv[0],#/hdiv[0][0],
            color=colors[ke-1])
          if vname=='newice':# or vname=='ridge_ratio': # if first expt load obs
            ax.set_title('Ice growth',fontsize=14,fontweight='bold')
            ax.set_xlabel('(m/day)',fontsize=14,fontweight='bold')
            plt.ylim([0, 3E6])
          elif vname=='ridge_ratio': # if first expt load obs
            ax.set_title('Ridging ratio')
            ax.set_xlabel('% of (Ridged ice) / (Total ice)')

        ax.set_ylabel('N. of occurrences',fontsize=14,fontweight='bold')
        if ex==expt[-1]:
          for i in expt:
            ll.append(runs[i])
