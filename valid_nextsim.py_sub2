...
#Time
start_day  =31 # 6 vcorr serie initial day
start_month=5
start_year =2016
end_day    =1 #24 # bsie
end_month  =10  #8 sit
...
for serie_or_map in serie_or_maps:
  ...
  if serie_or_map==1: # series
    ...
    expt=[exp]
    plot_series=0; plot_video=0; plot_map=0; plot_vchoice=1;
    vnames=[ 'vcorr','sit'] 
    varrays=['siv'  ,'sit'] 
  else:
    ...
    for ex in expt:
      ...
      if plot_smap==1:
        ...
        for ts in range(0,len(time_obsixn)):  
          ...
          ax.set_title(time[t].strftime('%Y/%m/%d %H:%M')[0], loc = 'right')
