          ...
          #for ts in range(0,len(time_obsixn)):  
            ...
            for ex in expt:
              ...
              if ex==expt[-1]:
                ...
                ax[ke].set_title('Brittle model ice concentration',loc='left')
              else:
                ax[ke].set_title('Viscous-plastic model ice concentration',loc='left')
            ...
            if ts==len(time_obsixn)-1:  
              ...
              figname=path_fig+run+'/ice_conc_Rafa_Santana_'+format(time[t[0]].strftime('%Y_%m_%d_%H_%M'))+'.png'
              if save_fig==1:
                if os.path.exists(path_fig+run)==False:
                  os.mkdir(path_fig+run)
                print('Saving: '+figname)
