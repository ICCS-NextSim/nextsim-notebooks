...
def time_series_plot2(time, variable,variable2, mask, save_name, title): 
    ''' 
    plot the time serie mean of any Masked Array and save the figure
    Inputs : 
    - time (datetime array)
    ...
    plt.plot(time, mean, '.--',label='momentum-only')
    plt.plot(time, mean2, '.--',label='momemtum+thermodynamics')
    plt.xlabel('Time')
    plt.legend()                      
    plt.ylabel('Average value')
    plt.title(title)
